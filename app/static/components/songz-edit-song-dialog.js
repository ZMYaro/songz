			...
			<mwc-dialog heading="Edit song" scrimClickAction="" escapeKeyAction="${this.pending ? '' : 'close'}">
				<form>
					<fieldset ?disabled="${this.pending}">
						<label style="font-size: 100%; color: var(--on-background-color); display: inline-block;"><input type="radio" name="rating" value="-3" .checked="${this.song?.rating === -3}" />👎👎👎</label>
						...
						<label>
							📃 GDrive MD:
							<input type="text" name="gdrive-md" .value="${this.song?.gDriveMD || ''}" />
						</label>
						<label>
							🆎 Title:
